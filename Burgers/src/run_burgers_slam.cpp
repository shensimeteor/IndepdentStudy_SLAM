...
int main(int argc, char** argv) {
    ...
    for( int i=1; i<=bgslam.window_steps; i++){
        for(int j=0; j<nx; j++){
            if(i==1 || bg.getNumericOption() == NUMERIC_OPTION_FORWARD){
                ...
                problem.AddResidualBlock(cost_functionProc, NULL, &Xs[i-1][(j-1+nx)%nx], &Xs[i-1][j], &Xs[i-1][(j+1)%nx], &Xs[i][j]);
            } else {
                CostFunction* cost_functionProc = CostFunctorProc_frogleap::createAutoDiffCostFunction(proc_err_var, i, &bg);
                problem.AddResidualBlock(cost_functionProc, NULL, &Xs[i-2][j], &Xs[i-1][(j-1+nx)%nx], &Xs[i-1][j], &Xs[i-1][(j+1)%nx], &Xs[i][j]);
            }
        }
    }
    ...
    for(int i=0; i< obss.nobs; i++){
        ...
        problem.AddResidualBlock(cost_functionObs, NULL, &Xs[it][ix]);
    }
